...
{
	textures->texNum = info->map[wall->mapX][wall->mapY] - 1;
	if (wall->side == 0)
		textures->wallX = info->posY + wall->perpWallDist * wall->rayDirY;
	else
		textures->wallX = info->posX + wall->perpWallDist * wall->rayDirX;
...
void draw_wall(t_info *info, t_textures *textures, t_wall *wall)
{
	...
	while (wall->y++ < wall->drawEnd)
	{
		...
		textures->color = info->texture[textures->texNum][texHeight * textures->texY + textures->texX];
		if (wall->side == 1)
			textures->color = (textures->color >> 1) & 8355711;
		info->buf[wall->y][wall->x] = textures->color;
	}
}
...
void flor_wall2(t_info *info, t_textures *textures, t_wall *wall, t_flor *flor)
{
	...
	for(textures->y = wall->drawEnd + 1; textures->y < height; textures->y++)
	{
		...
		flor->currentFloorX = wall->weight * wall->floorXWall + (1.0 - wall->weight) * info->posX;
		flor->currentFloorY = wall->weight * wall->floorYWall + (1.0 - wall->weight) * info->posY;
		...
		info->buf[textures->y][wall->x] = (info->texture[textures->floorTexture][texWidth * flor->floorTexY + flor->floorTexX] >> 1) & 8355711;
		info->buf[height - textures->y][wall->x] = info->texture[6][texWidth * flor->floorTexY + flor->floorTexX];
	}
}